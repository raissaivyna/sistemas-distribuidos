...
json ProdutoVeterinario::toJson() const {
    json j = Produto::toJson(); // serializa os atributos da classe base
    j["registroMapa"] = this->getRegistroMapa();
    j["especieAlvo"] = this->getEspecieAlvo();
    j["viaAdministracao"] = this->getViaAdministracao();
    return j;
}
