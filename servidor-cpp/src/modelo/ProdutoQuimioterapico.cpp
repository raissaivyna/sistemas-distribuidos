...
json ProdutoQuimioterapico::toJson() const {
    json j = ProdutoVeterinario::toJson(); // chama o toJson da classe base
    j["principioAtivo"] = this->getPrincipioAtivo();
    j["concentracao"] = this->getConcentracao();
    j["classeTerapeutica"] = this->getClasseTerapeutica();
    j["retencaoCarencia"] = this->ehRetencaoCarencia();
    return j;
}
