...
json VacinaNaoPerecivel::toJson() const {
    json j = ProdutoBiologico::toJson();
    j["formaFarmaceutica"] = this->getFormaFarmaceutica();
    j["prazoValidadeMeses"] = this->getprazoValidadeMeses();
    j["temperaturaMax"] = this->getTemperaturaMax();
    return j;
}
