...
json ProdutoBiologico::toJson() const {
    json j = ProdutoVeterinario::toJson();
    j["tipoAgente"] = this->getTipoAgente();
    j["sorotipo"] = this->getSorotipo();
    j["numDoses"] = this->getNumDoses();
    return j;
}
